...
[![Web](https://img.shields.io/badge/Web-在线播放-black.svg)](https://tosencen.github.io/XMSLEEP/player.html)

[下载应用](#下载) • [功能特性](#功能特性) • [使用说明](#使用说明) • [网页版](#-网页版)
...
无需安装，浏览器中直接体验全部功能：**113 种自然声音自由混音 · 无缝循环 · 6 种语言 · 倒计时定时**，声音清单与手机 App 同步。

👉 **在线播放**：[tosencen.github.io/XMSLEEP/player.html](https://tosencen.github.io/XMSLEEP/player.html)

> App 内 设置 → 其他 → 网页版 也可一键直达并复制链接分享。
...
### 🌐 网页版
- **在线播放器**：浏览器即开即用，113 种自然声音自由混音、无缝循环、音量独立控制
- **多语言界面**：支持中文/英文/日文/韩文/俄文/繁体中文 6 种语言
- **倒计时定时**：支持自定义倒计时，定时停止，时间实时显示
- **正在播放面板**：实时查看/停止/调节音量，播放数量一目了然
- **加载优化**：首屏预取 + LRU 缓存，重播秒开
...
#### 🎨 新功能
- **网页版在线播放器**：新增 GitHub Pages 网页版，113 种自然声音自由混音、无缝循环、6 种语言界面，声音与 App 同步，无需安装即开即用
- **App 网页版入口**：设置页「其他」模块新增「网页版」入口，一键跳转在线播放器，支持复制链接分享
- **黑白模式主题**：新增黑白主题，极简视觉风格
- **番茄钟后台计时**：离开页面后计时继续走，标尺显示当前阶段时长
- **番茄设置弹窗卡片化**：卡片化弹窗 + 结束铃声试听 + 休息时长可调
- **通知栏冥想控制**：通知栏/锁屏可直接控制冥想音频播放
- **数量字符串复数化**：英文/俄语等多语言数量显示本地化（plurals）

#### ✨ 改进
- **权限收紧**：精简权限声明，降低隐私暴露面
- **Firebase 统计与崩溃上报**：匿名使用统计与崩溃收集，不收集个人身份信息
- **依赖升级**：okhttp 5.3.2、gson 2.11.0、androidx.media 1.8.0、core-splashscreen 1.2.0
- **资源与签名整理**
...
#### 🎨 新功能
- **番茄时钟结束铃声**：内置 4 种免版权铃声（钟声/叮咚/木琴/风铃），专注结束自动响铃
- **番茄时钟结束震动**：专注结束自动震动提醒，支持设置开关
- **番茄设置弹窗**：番茄页右上角齿轮入口，可设置结束铃声、完成描边动画、震动提醒，弹窗完整展开
- **铃声/震动响 3 次**：结束铃声与震动各重复 3 次后自动停止，提醒更明显
- **专注结束手动开始休息**：专注结束后不再自动进入休息，显示「开始休息」按钮由用户手动触发
- **冥想循环次数**：冥想详情页倒计时改为「循环次数」，支持循环 1/2/3 次后自动停止，与无限循环互斥
- **天气模式番茄入口**：天气模式右上角新增番茄时钟快捷入口

#### ✨ 改进
- **番茄设置弹窗多语言**：支持中文/英文/日文/韩文/俄文/繁体中文
- **震动实现优化**：改用系统 Vibrator 直接震动并补充 VIBRATE 权限，可靠性提升
- **版本号升级**：2.2.9 → 2.3.0
