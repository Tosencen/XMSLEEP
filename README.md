...
- ✨ **音量持久化**：所有音频的音量设置会在应用重启后自动保留
- 🔄 **智能交互优化**：悬浮按钮与预设弹窗互斥，避免同时展开
- 📱 **滚动体验优化**：在主题设置和收藏页面滚动时自动收起悬浮按钮
- 🎯 **启动体验优化**：应用启动时预设弹窗始终为收起状态
- 🎨 **UI 细节优化**：更新图标小红点位置调整，收藏图标更换为自定义星形图标
- 🔧 **更新机制优化**：移除应用启动时的自动更新弹窗，改为图标提示
