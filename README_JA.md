...
#### 🎨 新機能
- **毎日一言のランダム背景**：毎日一言ダイアログの背景に Bing の日替わり壁紙をランダム採用（メモリのみキャッシュ・ディスク不使用）。文字は下端揃え・濃いグラデーションで可読性向上。共有/保存ポスターも同画像を使用
- **不適切ワード除去**：毎日一言から自傷・重度の否定的表現（自殺・うつ等）を自動ブロック。API は再取得、ローカルはスキップ

#### ✨ 改善
- **毎日一言ダイアログ**：画面の 2/3 高さに固定し、長文はスクロール。更新バッジを右上に移動し角丸に

### 🆕 最新アップデート (v2.3.2)

#### 🎨 新機能
- **和風天気ソース（BYOK）**：天気はユーザー自身の和風天気キー（設定で Host + Key 入力）に対応。未設定時は Open-Meteo に自動フォールバック。都市単位の位置情報＋多言語

#### ✨ 改善
- **天気カード**：都市と日付を一行に統合（例「上海市 8月25日 火曜日」）し癒やしの文言の上に配置。ソースバッジを削除
- **座標オフセット**：WGS84 → GCJ02 変換で国内位置精度を向上
