...
[![Web](https://img.shields.io/badge/Web-オンライン再生-black.svg)](https://tosencen.github.io/XMSLEEP/player.html)

[ダウンロード](#ダウンロード) • [機能](#機能) • [使い方](#使い方) • [ウェブ版](#-ウェブ版)
...
インストール不要で、ブラウザからすべての機能を体験できます：**113 種類の自然音 · 自由なミックス · シームレスループ · 6 言語 · カウントダウンタイマー**。サウンドリストはアプリと同期しています。

👉 **オンラインで再生**: [tosencen.github.io/XMSLEEP/player.html](https://tosencen.github.io/XMSLEEP/player.html)

> アプリ内 設定 → その他 → ウェブ版 からもワンタップでアクセスでき、リンクをコピーして共有できます。
...
- **Bilibili ラジオ**: Bilibili ライブ配信ルームを検索・再生、お気に入りルームをピン留め可能
...
### 🌐 ウェブ版
- **オンラインプレイヤー**: ブラウザですぐに使用可能。113 種類の自然音を自由にミックス、シームレスループ、個別音量調整
- **多言語対応**: 中国語/英語/日本語/韓国語/ロシア語/繁体中国語の 6 言語
- **カウントダウンタイマー**: カスタムカウントダウンと自動停止、リアルタイム表示
- **再生中パネル**: 再生中のサウンドを一覧表示・停止・音量調整
- **読み込み最適化**: ファーストビュー先読み + LRU キャッシュで即時再生
...
- **Hilt** - 依存性注入フレームワーク
- **Lottie** - 高品質アニメーションレンダリング
- **Web Audio API** - ウェブ版のシームレスループ再生（サンプル精度）
...
#### 🎨 新機能
- **ウェブ版オンラインプレイヤー**: GitHub Pages のウェブ版を追加。113 種類の自然音、自由なミックス、シームレスループ、6 言語対応、サウンドリストはアプリと同期。インストール不要ですぐに利用可能
- **アプリのウェブ版入口**: 設定の「その他」モジュールに「ウェブ版」を追加。ワンタップでオンラインプレイヤーを開き、リンクコピーにも対応
- **白黒モードテーマ**: モノクロテーマを追加し、ミニマルなビジュアルに
- **トマトタイマーのバックグラウンド計時**: ページを離れても計時継続、スケールに現在のフェーズ時間を表示
- **トマト設定シートのカード化**: カードスタイルのシート + 終了音の試聴 + 休憩時間の調整可能
- **通知欄の瞑想コントロール**: 通知欄/ロック画面から瞑想オーディオを直接操作可能
- **数量の複数形対応**: 英語/ロシア語などの数量表示をローカライズ（plurals）

#### ✨ 改善
- **権限の厳格化**: 権限宣言を整理し、プライバシー露出を低減
- **Firebase 統計とクラッシュレポート**: 匿名の利用統計とクラッシュ収集。個人情報は収集しません
- **依存関係のアップグレード**: okhttp 5.3.2、gson 2.11.0、androidx.media 1.8.0、core-splashscreen 1.2.0
- **リソースと署名の整理**
...
#### 🎨 新機能
- **トマトタイマー終了音**: 4種類のフリーサウンド（チャイム/ディンドン/マリンバ/ウィンドチャイム）を内蔵。集中終了時に自動再生
- **トマトタイマー終了バイブ**: 集中終了時に自動で振動でお知らせ。設定でオン/オフ可能
- **トマト設定シート**: トマトページ右上の歯車から終了音・完了アニメーション・バイブを設定。シートは全画面展開
- **終了音/バイブ×3回**: 終了音とバイブはそれぞれ3回繰り返して自動停止
- **休憩の手動開始**: 集中終了後は自動で休憩に入らず、「休憩を開始」ボタンで手動スタート
- **瞑想の繰り返し回数**: 瞑想詳細ページのカウントダウンを「繰り返し回数」に変更。1/2/3回ループ後に自動停止。無限ループとは排他
- **天気モードにトマト入口**: 天気モード右上にトマトタイマーへのショートカットを追加

#### ✨ 改善
- **トマト設定の多言語化**: 中国語/英語/日本語/韓国語/ロシア語/繁体中国語に対応
- **バイブ実装の最適化**: システム Vibrator を直接使用し VIBRATE 権限を追加、信頼性を向上
- **バージョンアップ**: 2.2.9 → 2.3.0

### 🆕 最新アップデート (v2.2.6)

#### 🎨 新機能
- **Bilibili ラジオ**: Bilibili ライブ配信ルーム機能を追加。自習室、ホワイトノイズなどのカテゴリを検索可能
- **ピン留め**: よく使うルームをピン留めして素早く切替、配信していないルームはグレー表示

#### ✨ 改善
- **編集モード最適化**: ボトムシートを閉じる際にピン編集モードを自動的に解除
- **プリセットダイアログ改善**: プリセット名編集後にダイアログを閉じる際に編集状態をリセット
...
9. **Bilibili ラジオ**: ラジオページの検索ボタンから Bilibili ライブルームをカテゴリ検索、ピンアイコンでよく使うルームを保存

### 高度な機能
10. **フローティングボタン**: 再生中に表示、タップで再生リスト確認
11. **一括追加**: フローティングボタン展開中にプリセットへ一括追加
12. **スマート格納**: スクロールやタブ切替時に自動格納
