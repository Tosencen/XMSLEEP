...
- **B站電台**：支援搜尋和播放 Bilibili 直播電台，可收藏常用房間
...
#### 🎨 新功能
- **每日一言隨機每日圖**：彈窗背景改為 Bing 每日桌布（每次隨機取圖，僅記憶體快取、不佔磁碟）；文字底部對齊並加深漸變確保可讀；分享/儲存海報同樣使用每日圖
- **敏感詞過濾**：每日一言自動遮蔽「自殺」「憂鬱」等自我傷害/重度消極詞彙；API 命中則重試，本地名句自動跳過

#### ✨ 改進
- **每日一言彈窗**：固定為螢幕 2/3 高度，內容過長可捲動；重新整理角標移至右上並改為圓角樣式

### 🆕 最新更新 (v2.3.2)

#### 🎨 新功能
- **和風天氣源（BYOK）**：天氣支援使用者自建和風天氣 Key（設定內填寫 Host + Key），未配置時自動回退 Open-Meteo；城市級定位 + 多語言

#### ✨ 改進
- **天氣卡片**：城市與日期合併一行（如「上海市 8月25日 星期二」）並置於治癒標語之上；移除資料來源角標
- **座標偏移**：WGS84 → GCJ02 轉換，提升國內定位精度
...
#### 🎨 新功能
- **番茄結束鈴聲**：內建 4 種免版權鈴聲（鐘聲/叮咚/木琴/風鈴），專注結束自動響鈴
- **番茄結束震動**：專注結束自動震動提醒，支援設定開關
- **番茄設定彈窗**：番茄頁右上角齒輪入口，可設定結束鈴聲、完成描邊動畫、震動提醒，彈窗完整展開
- **鈴聲/震動響 3 次**：結束鈴聲與震動各重複 3 次後自動停止
- **專注結束手動開始休息**：專注結束後不自動進入休息，顯示「開始休息」按鈕由使用者手動觸發
- **冥想循環次數**：冥想詳情頁倒計時改為「循環次數」，支援循環 1/2/3 次後自動停止，與無限循環互斥
- **天氣模式番茄入口**：天氣模式右上角新增番茄時鐘快捷入口

#### ✨ 改進
- **番茄設定彈窗多語言**：支援中文/英文/日文/韓文/俄文/繁體中文
- **震動實作優化**：改用系統 Vibrator 直接震動並補充 VIBRATE 權限，可靠性提升
- **版本號升級**：2.2.9 → 2.3.0

### 🆕 最新更新 (v2.2.6)

#### 🎨 新功能
- **B站電台**：新增 Bilibili 直播電台功能，支援搜尋自習室、白噪音等分類，一鍵播放直播間聲音
- **圖釘收藏**：支援收藏常用直播間，快速切換，非直播房間自動置灰提示

#### ✨ 改進
- **編輯模式優化**：退出彈窗時自動關閉圖釘編輯模式，避免誤操作
- **預設彈窗改進**：編輯預設名稱後關閉彈窗時自動重置編輯狀態
...
9. **B站電台**：在電台頁面點擊搜尋按鈕，可按分類搜尋 Bilibili 直播間，點擊圖釘圖示收藏常用房間

### 進階功能
10. **全域浮動按鈕**：當有聲音播放時，會出現浮動按鈕，點選可展開檢視正在播放的聲音
11. **批次新增到預設**：在浮動按鈕展開狀態下，可以將正在播放的聲音批次新增到目前預設
12. **智慧收起**：滾動頁面或切換標籤時，浮動按鈕會自動收起，避免遮擋內容
...
本應用重視您的隱私保護。為改進產品體驗，應用會收集**匿名**使用統計（如活躍用戶數、崩潰報告），**不會**收集任何個人身份資訊。
